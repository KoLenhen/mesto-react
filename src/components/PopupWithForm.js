...
function PopupWithForm(props) {
    ...
    return (
        <div className={`popup popup_type_${props.name} ${props.isOpen && 'popup_opened'}`}>
            <form action="index.html" name={`profile${props.name}`} noValidate
                ...
                {props.children}
                <button type="submit" className={`popup__btn_disabled popup__btn popup__btn_type_${props.name}`} >{props.btnText}</button>
            </form>
        </div>
    )
